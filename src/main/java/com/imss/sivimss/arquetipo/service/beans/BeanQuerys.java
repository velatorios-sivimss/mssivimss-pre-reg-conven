...
public class BeanQuerys {
	...
	public String queryDocsEmpresa(Integer idPreReg) {
		...
		query.append(
				" JOIN (SELECT LPAD(COUNT(SVDCPT.ID_CONVENIO_PF),4,'0') AS totalDocs, MAX(SVDCPT.ID_VALIDACION_DOCUMENTO) AS idDoc FROM SVC_VALIDA_DOCS_CONVENIO_PF SVDCPT");
		query.append(
				"       WHERE (SVDCPT.REF_DOC_INE_AFILIADO IS NOT NULL OR SVDCPT.REF_DOC_INE_AFILIADO != '') AND SVDCPT.ID_CONVENIO_PF = "
						+ idPreReg + " ) TD on TD.idDoc = SVDCP.ID_VALIDACION_DOCUMENTO");
		query.append(" UNION");
		query.append(
