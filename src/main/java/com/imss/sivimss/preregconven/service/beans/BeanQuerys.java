package com.imss.sivimss.preregconven.service.beans;
...
public class BeanQuerys {
	...
	private static final String CONTRAPAQ = "JOIN SVT_CONTRA_PAQ_CONVENIO_PF SCPCP ON SCPCP.ID_CONVENIO_PF = SCP.ID_CONVENIO_PF\r\n";
	private static final String FROM_VALIDA_DOCS = " FROM SVC_VALIDA_DOCS_CONVENIO_PF SVDCP";
	private static final String LPAD_COUNT = " JOIN (SELECT LPAD(COUNT(SVDCPT.ID_CONVENIO_PF),4,'0') AS totalDocs, MAX(SVDCPT.ID_VALIDACION_DOCUMENTO) AS idDoc FROM SVC_VALIDA_DOCS_CONVENIO_PF SVDCPT";
	private static final String TD_ID_DOC = " ) TD on TD.idDoc = SVDCP.ID_VALIDACION_DOCUMENTO";
	private static final String CVE_CURP = "		SP.CVE_CURP AS curp, ";
	private static final String WHERE_ID_PLAN = " WHERE SPS.ID_PLAN_SFPA = ";
	...
	public String queryPreRegistros(RequestFiltroPaginado request) {
		...
		queryPers.append(
				"SELECT SPS.ID_PLAN_SFPA AS idConvenioPlan, IFNULL(SPS.NUM_FOLIO_PLAN_SFPA, '') AS folioConvenio, STC.DES_TIPO_CONTRATACION AS tipoContratacion , IFNULL(SP.CVE_RFC, '') AS rfc,");
		queryPers.append(
				" CONCAT(SP.NOM_PERSONA, ' ', SP.NOM_PRIMER_APELLIDO , ' ', SP.NOM_SEGUNDO_APELLIDO) AS NombreAfiliadoTitular, SP2.REF_PAQUETE_NOMBRE AS tipoPaquete , 'p' AS tipoConvenio, 'PA' tipo, CAST(SPS.IND_ACTIVO AS int) AS indActivo ");
		...
		queryEmp.append(
				"SELECT SCP.ID_CONVENIO_PF AS idConvenioPlan, IFNULL(SCP.DES_FOLIO, '') AS folioConvenio, 'Empresa' AS tipoContratacion, IFNULL(SECP.CVE_RFC, '') AS rfc,");
		queryEmp.append(" SECP.REF_NOMBRE AS NombreAfiliadoTitular, ( SELECT \r\n"
				+ "GROUP_CONCAT(SP2.REF_PAQUETE_NOMBRE)\r\n"
				+ "FROM \r\n"
				+ "SVT_CONVENIO_PF SCP\r\n"
				+ CONTRAPAQ
				+ "JOIN SVT_PAQUETE SP2 ON SP2.ID_PAQUETE = SCPCP.ID_PAQUETE\r\n"
				+ "WHERE\r\n"
				+ "SCP.ID_CONVENIO_PF = idConvenioPlan\r\n"
				+ ")AS tipoPaquete, 'e' AS tipoConvenio , 'PF-Empresa' tipo,  CAST(SCP.IND_ACTIVO AS int) AS indActivo ");
		...
		queryEmpPer.append(
				"SELECT SCP.ID_CONVENIO_PF AS idConvenioPlan, IFNULL(SCP.DES_FOLIO, '') AS folioConvenio, 'Persona' AS tipoContratacion, IFNULL(SP.CVE_RFC, '') AS rfc,");
		queryEmpPer.append(
				" CONCAT(SP.NOM_PERSONA, ' ', SP.NOM_PRIMER_APELLIDO , ' ', SP.NOM_SEGUNDO_APELLIDO) AS NombreAfiliadoTitular, ( SELECT \r\n"
						+ "GROUP_CONCAT(SP2.REF_PAQUETE_NOMBRE) \r\n"
						+ "FROM \r\n"
						+ "SVT_CONVENIO_PF SCP\r\n"
						+ CONTRAPAQ
						...
						+ "WHERE\r\n"
						+ "SCP.ID_CONVENIO_PF = idConvenioPlan\r\n"
						+ ")AS tipoPaquete, 'ep' AS tipoConvenio , 'PF Persona' tipo,  CAST(SCP.IND_ACTIVO AS int) AS indActivo ");
		queryEmpPer.append(" FROM SVT_CONVENIO_PF SCP \r\n"
				+ CONTRAPAQ
				+ "JOIN SVC_CONTRATANTE SC ON SC.ID_CONTRATANTE = SCPCP.ID_CONTRATANTE  \r\n"
				+ "LEFT JOIN SVC_PERSONA SP ON SP.ID_PERSONA = SC.ID_PERSONA ");
		...
		// || (request.getConvenioPSFPA() != null && request.getConvenioPF() != null &&
		// request.getRfc() != null) || request.getRfc() != null
	...
	public String queryPreRegPersonasEmpresa(Integer idPreReg) {
		StringBuilder query = new StringBuilder();
		query.append(
				"SELECT SC.ID_CONTRATANTE AS idContratante, IFNULL(SC.CVE_MATRICULA, '') AS matricula, IFNULL(SPE.CVE_RFC, '') AS rfc, IFNULL(SPE.CVE_CURP, '') AS curp,");
		query.append(
				" SPE.NOM_PERSONA AS nombre, SPE.NOM_PRIMER_APELLIDO AS primerApellido, SPE.NOM_SEGUNDO_APELLIDO AS s egundoApellido, SD2.REF_CALLE AS calle,");
		query.append(
				" SD2.NUM_EXTERIOR AS numExt, SD2.NUM_INTERIOR AS numInt, SD2.REF_CP AS cp, SD2.REF_COLONIA AS colonia, SD2.REF_MUNICIPIO AS municipio, SD2.REF_ESTADO AS estado,");
		query.append(
				" SPE.ID_PAIS AS idPais, SPA.DES_PAIS AS pais, SPE.REF_CORREO AS correo, SPE.REF_TELEFONO AS telefono, SCPCP.IND_ENFERMEDAD_PREXISTENTE AS idenferpre");
	...
	public String queryDocsEmpresa(Integer idPreReg) {
		StringBuilder query = new StringBuilder();
		query.append(
				" SELECT TD.idDoc, TD.totalDocs AS numeroDocumento, 'INE del afiliado' AS tipoDocumento, SVDCP.REF_UBICACION_INE AS nombreDoc, SVDCP.REF_DOC_INE_AFILIADO AS documento");
		query.append(FROM_VALIDA_DOCS);
		query.append(
				LPAD_COUNT);
		query.append(
				"       WHERE (SVDCPT.REF_DOC_INE_AFILIADO IS NOT NULL OR SVDCPT.REF_DOC_INE_AFILIADO != '') AND SVDCPT.ID_CONVENIO_PF = "
						+ idPreReg + TD_ID_DOC);
		query.append(" UNION");
		query.append(
				" SELECT TD.idDoc, TD.totalDocs AS numeroDocumento, 'CURP del afiliado' AS tipoDocumento, SVDCP.REF_UBICACION_CURP AS nombreDoc, SVDCP.REF_DOC_CURP_AFILIADO AS documento");
		query.append(FROM_VALIDA_DOCS);
		query.append(
				LPAD_COUNT);
		query.append(
				"       WHERE (SVDCPT.REF_DOC_CURP_AFILIADO IS NOT NULL OR SVDCPT.REF_DOC_CURP_AFILIADO != '')  AND SVDCPT.ID_CONVENIO_PF = "
						+ idPreReg + TD_ID_DOC);
		query.append(" UNION");
		query.append(
				" SELECT TD.idDoc, TD.totalDocs AS numeroDocumento, 'RFC del afiliado' AS tipoDocumento, SVDCP.REF_UBICACION_RFC AS nombreDoc, SVDCP.REF_DOC_RFC_AFILIADO AS documento");
		query.append(FROM_VALIDA_DOCS);
		query.append(
				LPAD_COUNT);
		query.append(
				"       WHERE (SVDCPT.REF_DOC_RFC_AFILIADO IS NOT NULL OR SVDCPT.REF_DOC_RFC_AFILIADO != '') AND SVDCPT.ID_CONVENIO_PF = "
						+ idPreReg + TD_ID_DOC);
	...
	public String queryPreRegistrosXPersona(Integer idPreReg) {
		String query = ""
				...
				+ WHERE_ID_PLAN + idPreReg;
		log.info(query);
		return query;
	}

	public String queryCatPaquetes() {
		String query = "SELECT SP.ID_PAQUETE AS idPaquete, SP.REF_PAQUETE_NOMBRE AS nombrePaquete FROM SVT_PAQUETE SP WHERE SP.IND_ACTIVO = 1 and  SP.IND_ONLINE = 1 ";
		log.info(query);
		return query;
	}
